...
-- 1. 조인의 종류
-- (1) inner join
-- : 두 테이블에서 join "조건에 일치하는 행만" 결합하여 반환한다. -> NULL 값은 제외
SELECT 
    a.menu_name,
    b.category_name
FROM
...
JOIN tbl_category b ON (a.category_code = b.category_code);  -- (조인 조건)
...
FROM
    employee a
JOIN department b ON a.dept_code = b.dept_id;  -- 의미적으로는 같은 데이터를 가지고 있지만, 컬럼명이 다른 테이블이 있으므로 잘 구별하여 사용해야 한다.
...
FROM
    tbl_menu a
JOIN tbl_category b USING (category_code); -- join 컬럼명이 같아야 한다.
...
-- right join
-- : 오른쪽 테이블의 모든 행을 유지하고 왼쪽 테이블에서 조건이 일치하는 행이 있으면 결합한다. (방향만 반대)
...
FROM
    employee a
RIGHT JOIN department b ON a.dept_code = b.dept_id;  -- department 테이블 기준

-- (3) cross join
-- : 두 테이블의 모든 가능한 조합을 반환한다.
--   예를 들어 a 테이블의 행이 10개 b 테이블의 행이 5개이면 10 x 5 = 50개의 행이 반환된다.
SELECT 
    a.menu_name,
    b.category_name
FROM
...
-- (1) self join
-- 같은 테이블을 두 번 이상 참조하어 동일 테이블 내의 행과 행 사이의 관계를 조회하는 join 방식

-- 카테고리 상위/하위 관계 조회
SELECT
    a.category_name as sub_category,
    b.category_name as parent_categoty  -- 똑같은 category_name 이지만 별칭을 다르게 하여 사용
FROM
    tbl_category a  -- 하나의 테이블 안에서 self join
JOIN
...
FROM
    employee a
LEFT JOIN employee b ON a.manager_id = b.emp_id;  -- (조인 조건) 헷갈리지 말기 !
...
-- 사원명, 근무 지역명, 근무 국가명 검색 (부서 없는 사원 제외 -> inner join)
SELECT
    a.emp_name 사원명,
    c.local_name 근무지역명,

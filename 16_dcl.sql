...
-- 권한 확인
SHOW GRANTS FOR 'user01'@'%';  -- 접속할 수는 있지만 특정 권한은 없는 상태

-- 권한 부여
-- menudb 데이터베이스의 모든 객체를 조회할 수 있는 권한을 user01에게 부여
...
-- 여러 권한 한 번에 부여
-- "메뉴 테이블"에 대한 삽입, 수정 권한을 user01 계정에 부여 (세분화된 권한 부여 가능)
...
INSERT INTO tbl_menu VALUES (NULL, '메뉴명', 1000, 4, 'Y');  -- 오로지 SELECT 권한만 가지고 있다.

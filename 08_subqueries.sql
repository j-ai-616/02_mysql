...
-- 메인 쿼리에서 참조하려면 반드시 alias를 작성해야 한다.
SELECT
    MAX(menu_count)
FROM  -- FROM절의 서브쿼리는 마치 테이블처럼 사용된다.(파생테이블)
    (
        SELECT
            category_code,
            COUNT(*) as menu_count
...
FROM
    tbl_category a 
WHERE
    EXISTS (  -- 결과가 존재하면 true, 존재하지 않으면 false를 반환.
        SELECT  -- select절의 컬럼에 뭐가 들어가는지는 중요하지 않다. 아무값이나 써도 되니까 그냥 1이라고 썼다.
